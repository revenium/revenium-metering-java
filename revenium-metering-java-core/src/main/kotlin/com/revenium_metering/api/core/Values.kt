...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { ReveniumMeteringInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw ReveniumMeteringInvalidDataException("`$name` is not set")
            is JsonNull -> throw ReveniumMeteringInvalidDataException("`$name` is null")
            else -> throw ReveniumMeteringInvalidDataException("`$name` is invalid, received $this")
        }
